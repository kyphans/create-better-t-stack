...
- **Programmatic API (`create`, `createVirtual`):**
  Tệp này còn chứa các hàm _export module_. Nhờ đó, nếu một dự án TypeScript bên ngoài cài `create-kps` như thư viện npm, họ có thể xài `import { createVirtual } from "create-kps";` để gen source code trên biến ram mà không cần bật terminal hỏi đáp.
