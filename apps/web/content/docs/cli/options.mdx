...
```bash
create-kps --package-manager bun
```
...
create-kps my-app --yes --directory-conflict overwrite

# Safely create a new directory name if it exists
create-kps my-app --yes --directory-conflict increment
```
...
create-kps --disable-analytics

# Enable analytics collection (default)
create-kps --no-disable-analytics
```

Analytics help improve KPS by providing insights into usage patterns. When disabled, no data is collected or transmitted.
...
```bash
create-kps --database postgres --orm drizzle
```
...
```bash
create-kps --database postgres --db-setup neon
```
...
```bash
create-kps --backend hono --runtime bun
```
...
```bash
create-kps --auth better-auth
create-kps --auth clerk
create-kps --auth none
```
...
- `mcp`: Install MCP servers, including KPS itself, with add-mcp
...
```bash
create-kps --addons pwa biome husky
```
...
```bash
create-kps --web-deploy cloudflare
```
...
```bash
create-kps --server-deploy cloudflare
```
...
- **macOS**: `~/Library/Application Support/kps/history.json`
- **Linux**: `~/.local/share/kps/history.json`
- **Windows**: `%LOCALAPPDATA%\kps\Data\history.json`
